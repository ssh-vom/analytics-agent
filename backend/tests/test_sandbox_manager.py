...
class FakeRunner:
    def __init__(self, delay: float = 0.05) -> None:
        self.delay = delay
        self.calls: list[tuple[str, str, int, float, float]] = []

    async def execute(self, worldline_id: str, code: str, timeout_s: int):
        start = time.monotonic()
        await asyncio.sleep(self.delay)
        end = time.monotonic()
        self.calls.append((worldline_id, code, timeout_s, start, end))
        return {
            "stdout": code,
            "stderr": "",
            "error": None,
...
class SandboxManagerTests(unittest.TestCase):
    ...
    def test_worldline_sticky_handle(self):
        async def case():
            manager = SandboxManager(FakeRunner())
    ...
    def test_same_worldline_is_serialized(self):
        async def case():
            ...
            _, _, _, s1, e1 = runner.calls[0]
            _, _, _, s2, _ = runner.calls[1]
